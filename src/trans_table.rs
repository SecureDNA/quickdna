...
impl TranslationTable {
    ...
    const AA_TRANSLATION_TABLES: &'static [Option<AminoAcid>; Self::LOOKUP_SIZE] =
        &Self::build_aa_tables(Self::TRANSLATION_TABLES);

    const fn build_aa_tables(
        translation_table: &'static [u8; Self::LOOKUP_SIZE],
    ) -> [Option<AminoAcid>; Self::LOOKUP_SIZE] {
        let mut table = [None; Self::LOOKUP_SIZE];
        let mut i = 0;
        while i < Self::LOOKUP_SIZE {
            table[i] = crate::amino_acid::ASCII_TO_AMINO_ACID[translation_table[i] as usize];
            i += 1;
        }
        table
    }

    fn aa_table(table_idx: usize) -> &'static [Option<AminoAcid>] {
        &Self::AA_TRANSLATION_TABLES
            [table_idx * Self::CODONS_PER_TABLE..(table_idx + 1) * Self::CODONS_PER_TABLE]
    }
    ...
    pub fn to_fn(&self) -> impl Fn(Codon) -> Option<AminoAcid> {
        let table = Self::aa_table(self.table_index());
        move |codon| {
